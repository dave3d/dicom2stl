...
def regularize(img, maxdim=-1, verbose=False):
  ...
  vcorners = [ [0, 0, 0], [dims[0], 0, 0],
               [0, dims[1], 0], [dims[0], dims[1], 0],
               [0, 0, dims[2]], [dims[0], 0, dims[2]],
               [0, dims[1], dims[2]], [dims[0], dims[1], dims[2]] ]
  ...
  newimg = sitk.Resample(img, newdims, sitk.Transform(),
                         sitk.sitkLinear,
                         mins,
...
if __name__ == "__main__":
  ...
  if len(sys.argv) == 1:
    ...
    img = sitk.GaussianSource(sitk.sitkUInt8, size=[256, 256, 74],
                          sigma = [20, 20, 5],
                          mean=[128, 128, 37],
                          scale=255,
                          )
    img.SetSpacing([.4, .4, 2.0])
